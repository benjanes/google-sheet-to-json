...
describe('googleResponseToArray', function() {
	...
	it('should have an object with a key, "c", at each index', function() {
		const eachRowHasCKey = sheetArr.reduce((hasCKey, row) => {
			return hasCKey ? row.hasOwnProperty('c') : false;
		}, true);
		expect(eachRowHasCKey).toEqual(true);
	});

	it('should have an array as a value at the key "c"', function() {
		expect(Array.isArray(sheetArr[0].c)).toEqual(true);
	});
});
